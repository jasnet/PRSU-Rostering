...
def employee_scheduling_constraints(constraint_factory: ConstraintFactory):
    # list of all constraints to be sent to the model
...
def required_skill(constraint_factory: ConstraintFactory):
    # penalize if the required skill in shift is not in the employee's skill set
...
def no_overlapping_shifts(constraint_factory: ConstraintFactory):
    # Shifts should not overlap, if they do then penalize the solution with the overlapped 
    # number of minutes, ie, the overlap is of how many minutes.
